...
{
    uint16_t numEntries;
    void (**callbacks)(void*,void*,int*,double*,float*,char*,long*,char*,char*);
    void (**callback)(char*,char*);
    void (**callbac)(void*,void*,int*,double*,float*,char*,long*,char*,char*);
} HotCallTable;

#define HOTCALL_INITIALIZER  {0, SGX_SPINLOCK_INITIALIZER, NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0, true, false, false, false }
...
static inline int HotCall_requestCall( HotCall* hotCall, uint16_t callID, void *data,int *intArray,double *doubleArray,float *floatArray,char *charArray,long *longArray,char *byteArray,char *uuid,void *rei)
{
    int i = 0;
	    ...
	    //int* intArray = hotCall->intArray;
   	   ...
   	   // char* byteArray = hotCall->byteArray;
            ...
            //if( callID < callTable->numEntries ) {
                ...
                if(callID == 0){//branch
                	callTable->callbacks[ callID ]( hotCall->data,hotCall->rei,hotCall->intArray,hotCall->doubleArray,hotCall->floatArray,hotCall->charArray,hotCall->longArray,hotCall->byteArray,hotCall->uuid);
	   	}
		if(callID == 1 || callID == 2){
			callTable->callback[ callID ](hotCall->uuid,hotCall->charArray);
		}
