...
public class TeraInputFormat extends FileInputFormat<Text,Text> {
  ...
  static class TextSampler implements IndexedSortable {
    ...
    Text[] createPartitions(int numPartitions) {
      ...
      double stepSize = numRecords / (double) numPartitions;
      System.out.println("Step size is " + stepSize);
      Text[] result = new Text[numPartitions-1];
System.out.println("A");
      for(int i=1; i < numPartitions; ++i) {
        result[i-1] = records.get((int)Math.round(stepSize * i));
      }
System.out.println("B");
  ...
  public InputSplit[] getSplits(JobConf conf, int splits) throws IOException {
	// int conf_hashcode = conf.hashCode();
	 // System.out.println(lastConf.toString());
	JobConf zy = conf;
	JobConf st = lastConf;
	//int zy = conf.hashCode();
	//int st = lastConf.hashCode();
    if (zy == st) {
      return lastResult;
    }
    lastConf = conf;
    //if(conf.hashCode() == lastConf.hashCode()){
    //	System.out.println("if conf!=lastconf,there hashcode is also equal");
    //}
